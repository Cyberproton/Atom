...
final class DefaultServerThreadLock implements ServerThreadLock {
    ...
    DefaultServerThreadLock(SchedulerManager schedulerManager) {
        // already sync - just countdown on obtained & return
        if (ThreadContext.ofCurrentThread() == ThreadContext.SYNC) {
        ...
        schedulerManager.sync().execute(this::signal);
